...
for sample_size in sample_sizes:
	...
	for sub in sub_idxs:
		...
		for ratio_generate in [1, 2, 4]:
			...
			# =============================================================================
			x_test = np.expand_dims(test_data[sub]['xtest'], axis=1)
			y_test = test_data[sub]['ytest']
	...
	with open(gan_type+'_ratio_'+str(ratio_generate)+'_sample-size_'+str(sample_size)+'_subject-specific-results.txt', 'w') as f:
		f.write(str(accuracies))

	
	all_accuracies[str(sample_size)].append(accuracies)
	del data 
with open(gan_type+'_subject_specific_results_ALL.txt', 'w') as f:
