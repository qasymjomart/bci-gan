...
from data_import import CustomEEGDataset
...
def gan_test(real_data, generated_data, classifier):
	...
	x_train, y_train = real_data['x'], real_data['y']
	x_test, y_test = generated_data['x'], generated_data['y']
	...
	if classifier == 'LDA':
		...
		print('SVM input shape: ', x_train.shape)
		print('SVM test data shape: ', x_test.shape)
		x_train = np.reshape(x_train, (x_train.shape[0], x_train.shape[1]*x_train.shape[2]))
		x_test = np.reshape(x_test, (x_test.shape[0], x_test.shape[1]*x_test.shape[2]))
		clf = LinearSVC()
		clf.fit(x_train, y_train)
		...
		accuracy_m = correct/total
		accuracy = accuracy_score(y_test, y_pred)
		print(accuracy, accuracy_m, y_pred.shape, y_test.shape)
		assert accuracy == accuracy_m
	
	elif classifier == 'DCNN':
		x_train = torch.unsqueeze(torch.from_numpy(x_train), axis=1)
		train_dat = CustomEEGDataset(x_train, torch.from_numpy(y_train).type(dtype = torch.long))
		train_loader = DataLoader(train_dat, batch_size = 32, shuffle = True)
		
		x_test = torch.unsqueeze(torch.from_numpy(x_test), axis=1)
		test_dat = CustomEEGDataset(x_test, torch.from_numpy(y_test).type(dtype = torch.long))
